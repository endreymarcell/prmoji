...
export class SlackClient {
    ...
    addEmoji(name, channel, timestamp) {
        logger.info('[slack] Slack client called with', JSON.stringify({emoji: name, channel, timestamp}, null, 2))
        return this.client.reactions.add({name, channel, timestamp}).catch((error) => logger.error(error))
    }

    sendMessage(message, channel) {
        logger.info('[slack] Slack client called with:', JSON.stringify({channel, message: '(hidden)'}, null, 2))
